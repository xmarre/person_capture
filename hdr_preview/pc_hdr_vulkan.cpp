...
struct pc_hdr_context {
    ...
    // Push constants: video and output dimensions
...
static void createSwapchain(pc_hdr_context* ctx, int widgetWidth, int widgetHeight) {
    ...
    ctx->pushConstants.outWidth = static_cast<int>(ctx->swapchainExtent.width);
    ctx->pushConstants.outHeight = static_cast<int>(ctx->swapchainExtent.height);
